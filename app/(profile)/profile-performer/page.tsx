...
import { getResumesByAuthor } from "@/app/actions/getResumesByAuthor";
...
export default function ProfilePage() {
  ...
  const createdResumesQuery = useQuery({
    queryKey: ["createdResumes"],
    async queryFn() {
      return await getResumesByAuthor({ id: session!.user.id });
    },
    ...
    <main className="overflow-hidden">
      ...
      <div className="max-w-[calc(1100px-2rem)] px-4 mx-auto -translate-y-24 flex flex-col gap-9">
        ...
        <Tabs value={tabs} onValueChange={(value) => setTabs(value as TabsMap)}>
          ...
          <TabsContent value={TabsMap.Resume}>
            <div>
              <header className="flex items-center justify-end mb-2">
                <Button type="button" asChild>
                  <Link href={"create-resume"}>Создать</Link>
                </Button>
              </header>
              <div className="flex flex-wrap items-center justify-center gap-4">
                {!createdResumesQuery.isLoading ? (
                  createdResumesQuery.data &&
                  createdResumesQuery.data.length > 0 ? (
                    createdResumesQuery.data?.map((resume) => (
                      <ResumeItem
                        key={resume.id}
